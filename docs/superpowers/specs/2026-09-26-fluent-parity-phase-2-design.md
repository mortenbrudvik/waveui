...
Story `stories/Presence.stories.tsx` (title `Components/Motion/Presence`): `FadeAndSlide` (a Button toggles a card with `transition-[opacity,translate] duration-wave-normal ease-wave-decelerate-mid starting:opacity-0 starting:translate-y-1 data-[presence=exiting]:opacity-0 data-[presence=exiting]:translate-y-1 data-[presence=exiting]:duration-wave-fast data-[presence=exiting]:ease-wave-accelerate-mid motion-reduce:transition-none`) and `KeepMounted` (`unmountOnExit={false}`, a text field whose content survives).
