...
A collapsed `Nav.Category` that contains the current page marks its toggle button with `aria-current="true"` (not `"page"`: the button is not the page's link) and the selected look.
