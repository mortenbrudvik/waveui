...
describe('Link', () => {
  ...
  it('renders as an anchor by default', () => {
    render(<Link href="#" data-testid="link">Go</Link>);
    expect(screen.getByTestId('link').tagName.toLowerCase()).toBe('a');
  });

  it('renders as a different element via as prop', () => {
    render(<Link as="button" data-testid="link">Go</Link>);
  ...
  it('applies standalone variant', () => {
    render(<Link href="#" variant="standalone" data-testid="link">Go</Link>);
    expect(screen.getByTestId('link').className).toContain('font-semibold');
  });

  it('applies subtle variant', () => {
    render(<Link href="#" variant="subtle" data-testid="link">Go</Link>);
  ...
  it('prevents click when disabled', async () => {
    ...
    const onClick = vi.fn();
    render(<Link href="#" disabled onClick={onClick}>Go</Link>);
    await user.click(screen.getByText('Go'));
    expect(onClick).not.toHaveBeenCalled();
  });

  it('sets aria-disabled when disabled', () => {
    render(<Link href="#" disabled data-testid="link">Go</Link>);
